...
- 容器最大宽度：`max-w-6xl`（卷首语/更多产品）、`max-w-4xl`（封底 CTA）、`max-w-7xl`（产品页）。
- 水平内边距：`px-5 md:px-6`。
...
> 素材已从 `apps/miniapp/src/assets/` 复制到 `web/landing/assets/` 目录，网站统一引用本地路径。原始素材发生变更时，需同步复制更新。

| 素材 | 路径 | 使用位置 |
|------|------|----------|
| Logo 96px | `web/landing/assets/logo/thumb-96.png` | 导航栏、更多产品卡片 |
| Logo 400px | `web/landing/assets/logo/logo-400.png` | 封底 CTA |
| 城市夜空插画 | `web/landing/assets/theme-illust/night.png` | 封面底部 |
| 咖啡时光插画 | `web/landing/assets/theme-illust/coffee.png` | 卷首语金句卡 |
| 城市公园插画 | `web/landing/assets/theme-illust/park.png` | 卡片墙 |
| 朋友相聚插画 | `web/landing/assets/theme-illust/gather.png` | 时间轴 |
| 展览记忆插画 | `web/landing/assets/theme-illust/exhibit.png` | 旅程地图 |
| 旅程空状态插画 | `web/landing/assets/empty-state/journey.png` | 旅程地图卡片内 |
...
### 7.2 文件位置
- 入口文件：`web/landing/index.html`
- 样式文件：`web/landing/styles.css`
- 素材目录：`web/landing/assets/`
- 验证脚本：`scripts/verify-landing.sh`
...
### 8.2 生产部署
1. 确认 `web/landing/assets/` 下所有图片素材已被复制到部署目录。
2. 将 `web/landing/index.html` 作为入口文件部署到静态托管（如 Vercel、Cloudflare Pages、Nginx）。
...
本网站已实现并验证通过。后续可考虑：
- 替换封底「小程序码占位」为真实小程序二维码
- 添加 Open Graph / Schema.org 等 SEO 标记
- 接入页面分析埋点
- 部署到生产静态托管服务
...
- [x] 目录结构：`web/landing/`
- [x] 素材自包含：`web/landing/assets/`（8 个图片文件）
- [x] 样式文件拆分：`web/landing/styles.css`
- [x] 入口 HTML 规范化：`web/landing/index.html`
- [x] 移动端菜单交互与可访问性优化
- [x] 本地预览验证脚本：`scripts/verify-landing.sh`
- [x] 本地预览验证通过
